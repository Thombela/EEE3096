...
int main(void){
  ...
  while (1)
  {
	  HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_11);
		...
		//Determine the amount of additional zeros we need by determining how many bits the adc_val is
		while (1){
		...
		for (uint32_t i =0; i < 13; i++){
		  if (*(val + i) == 0){
			  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_11, GPIO_PIN_RESET);  // turns the LED "off"
			  HAL_Delay (500); // wait for 500 ms
		  }
		  else{
			  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_11, GPIO_PIN_SET);   // turns the LED "on"
			  HAL_Delay (500); // wait for 500 ms
		  }
		}
  }
